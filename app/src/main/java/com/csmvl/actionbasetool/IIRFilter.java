...
public class IIRFilter {
    ...
    private int order;
    private int prototype, filterType, freqPoints = 3;
    private double fp1, fp2, fN, ripple, rate;
    ...
    private void locatePolesAndZeros() {
        ...
        switch (filterType) {
            ...
            case BP:
                ...
                /*
                 * check this bit ... needs value for gp to adjust critical freqs if
                 * (prototype == BUTTERWORTH) { f4 =
    ...
    public double[] filterGain() {
        ...
        double gMax = -100.0f;
        double sc = 10.0f / (double) Math.log(10.0f);
        ...
        for (int i = 0; i <= freqPoints; i++) {
            ...
            g[i] = sc * (double) Math.log((sqr(sac) + sqr(sas)) / (sqr(sbc) + sqr(sbs)));
        ...
        double normFactor = (double) Math.pow(10.0, -0.05 * gMax);
